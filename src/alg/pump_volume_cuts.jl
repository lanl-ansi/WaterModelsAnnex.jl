...
function _sum_remaining_pump_flows(wm::WM.AbstractWaterModel, nws::Vector{Int64}, source_pumps::Array)
    return sum(sum(WM.var(wm, nw, :q_pump, a) for a in source_pumps) for nw in nws)
end


function _sum_remaining_reservoir_flows(wm::WM.AbstractWaterModel, nws::Vector{Int64})
    return sum(sum(WM.var(wm, nw, :q_reservoir, a) for a in WM.ids(wm, nw, :reservoir)) for nw in nws)
end


function _sum_remaining_demands(wm::WM.AbstractWaterModel, nws::Vector{Int64})
