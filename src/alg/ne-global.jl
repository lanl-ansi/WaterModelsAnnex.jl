...
function ne_global(network_path::String, modifications_path::String,
    ...
    wf = WMs.build_model(network, WMs.NCNLPWaterModel, WMs.post_wf)
    wf_result = WaterModels.run_wf(network, WMs.NCNLPWaterModel, nlp_optimizer)
    ...
    ext = Dict(:num_breakpoints => 3)
    ne = WMs.build_model(network, WMs.MILPRWaterModel, WMs.post_ne, ext=ext)
    ...
    while !design_is_feasible && iteration_counter != max_iterations
        iteration_counter += 1
        ne_solution = WMs.optimize_model!(ne, mip_optimizer)
        objective_value = ne_solution["objective"]
        ...
        if !design_is_feasible
            ...
            add_no_good_cut!(ne, mip_optimizer)
        end
