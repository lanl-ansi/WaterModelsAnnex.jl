"""
This function takes in the WaterModels data dictionary and returns a dictionary of all
directional links in the water network that result from components on the network.
"""
...
"""
This function takes in the WaterModels data dictionary and returns a dictionary of all
directional links associated with pumps in the water network.
"""
function find_directional_pumps(data::Dict{String,<:Any})
    return Dict(a => [x["node_fr"], x["node_to"]] for (a, x) in data["pump"])
end 


"""
This function checks if the edge from current node to next node, if deleted, disconnects the
graph. If the graph is disconnected, it then checks if next_node is in the same component of
the graph as sink. If not, the edge fails the test. Otherwise, the edge passes the test.
"""
function component_check(
    sink::Int, current_node::Int, next_node::Int,
...
"""
function find_paths(
    start::Int, _end::Int, graph::LightGraphs.SimpleGraphs.AbstractSimpleGraph{Int64})
...
"""
function check_path_for_pump(
    path::Array{Any, 1}, pumps_dict::Dict{<:Any, <:Any}, node_map::Dict{String, Int64})
    status = false
    ...
    for k in range(1, stop = length(path) - 1)
        ...
        for (pump_id, pump) in pumps_dict
            pump_start = node_map[string(pump[1])]
            pump_end = node_map[string(pump[2])]
...
"""
This function checks to see if a path in the graph has a directional component on it making
it so water could not flow from the start of the path to end of the path along the path. 
"""
function check_path_for_directional_components(
    path::Array{Any, 1}, directional_components_dict::Dict{Any, Any},
    node_map::Dict{String, Int64})
    status = false
    ...
    for k in range(1, stop = length(path) - 1)
        ...
        for (component_id,component) in directional_components_dict
            component_start = node_map[string(component[1])]
            component_end = node_map[string(component[2])]
...
"""
This function checks to see if a path from a reservoir to the start of a pump in the graph
has a directional component or pumps on it, making it so water flowing from the start of the
path to end of the path along the path could either not happen or would be water that has
already passed through a pump.
"""
function check_path(
    path::Array{Any, 1}, pumps_dict::Dict{<:Any, <:Any},
    directional_components_dict::Dict{<:Any, <:Any}, node_map::Dict{String, Int64})
...
"""
This function goes through all the pumps on the water network and determines which ones are
source pumps and which ones are not and returns a list of source pumps.
"""
