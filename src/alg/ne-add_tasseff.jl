function add_system!(wm::WMs.GenericWaterModel)
    ...
    for (a, link) in wm.ref[:nw][n][:links]
        ...
        for r in 1:length(wm.ref[:nw][n][:resistance][a])
            ...
            lambda_p[a][r] = JuMP.@variable(wm.model, [k in 1:num_outer_cuts_p[a][r]], lower_bound = 0.0, base_name = "lambda_p[$(a)][$(r)]", start=0.0)
            lambda_n[a][r] = JuMP.@variable(wm.model, [k in 1:num_outer_cuts_n[a][r]], lower_bound = 0.0, base_name = "lambda_n[$(a)][$(r)]", start=0.0)
            ...
            lambda_p_definition[a][r] = JuMP.@constraint(wm.model, sum(lambda_p[a][r]) <= x_r_a)
            lambda_n_definition[a][r] = JuMP.@constraint(wm.model, sum(lambda_n[a][r]) <= x_r_a)
    ...
    for (i, reservoir) in wm.ref[:nw][n][:reservoirs]
        for (a, link) in filter(a -> i == a.second["f_id"], wm.ref[:nw][n][:links])
            fp += reservoir["head"] * (sum(qp[a]) - sum(qn[a]))
        end
    ...
    for (i, junction) in wm.ref[:nw][n][:junctions]
        fd += wm.var[:nw][n][:h][i] * junction["demand"]
    end

    strong_duality = JuMP.@constraint(wm.model, fp - fd >= 0.0)
end
